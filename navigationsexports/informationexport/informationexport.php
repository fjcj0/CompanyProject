...
if (!isset($_REQUEST['good_id'])) {
    header('Location: /company/navigationsexports/exports/exports.php');
    exit;
}
...
$export_id = (int) $_REQUEST['good_id'];
$stmt = $conn->prepare("SELECT * FROM exports WHERE id = ?");
$stmt->bind_param("i", $export_id);
$stmt->execute();
$data_export = $stmt->get_result();
$export = $data_export->fetch_assoc();
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>معلومات البضاعة المصدرة</title>
...
<body class="d-flex align-items-center justify-content-center flex-column">
    <h1>معلومات البضاعة المصدرة</h1>
    <img src="<?php echo '../../../' . $export['image']; ?>" alt="">
    <p>الاسم: <?php echo $export['name']; ?></p>
    <p>الكمية: <?php echo $export['quantity']; ?></p>
    <p>السعر: $<?php echo $export['price']; ?></p>
    <p>تاريخ التصدير: <?php echo $export['created_at']; ?></p>
    <p>المكان: <?php echo $export['place']; ?></p>
</html>