...
if (!isset($_REQUEST['good_id'])) {
    header('Location: /company/navigationsgoods/goods/goods.php');
    exit;
}
...
$good_id = (int) $_REQUEST['good_id'];
$stmt = $conn->prepare("SELECT * FROM goods WHERE id = ?");
$stmt->bind_param("i", $good_id);
$stmt->execute();
$data_good = $stmt->get_result();
$good = $data_good->fetch_assoc();
?>
<!DOCTYPE html>
...
<body class="d-flex align-items-center justify-content-center flex-column">
    <h1>معلومات البضاعة</h1>
    <img src="<?php echo '../../../' . $good['image']; ?>" alt="">
    <p>الاسم: <?php echo $good['name']; ?></p>
    <p>الكمية: <?php echo $good['quantity']; ?></p>
    <p>السعر: $<?php echo $good['price']; ?></p>
    <p>تاريخ التصدير: <?php echo $good['created_at']; ?></p>
    <p>المصدر: <?php echo $good['exporter']; ?></p>
    <p>المكان: <?php echo $good['place']; ?></p>
</html>