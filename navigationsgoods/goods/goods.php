...
$total_query = "SELECT COUNT(*) AS total FROM goods WHERE name LIKE '%$search_name%'";
$result = $conn->query($total_query);
$total_rows = $result->fetch_assoc()['total'];
$total_pages = ceil($total_rows / $records_per_page);
$goods_stmt = "SELECT * FROM goods WHERE name LIKE '%$search_name%' LIMIT $records_per_page OFFSET $offset";
$goods_result = $conn->query($goods_stmt);
?>
<!DOCTYPE html>
...
<head>
    ...
    <link rel="icon" href="https://img.icons8.com/?size=100&id=d7TXQ0O1UmL4&format=png&color=000000">
    <style>
...
<body class="d-flex align-items-center justify-content-center flex-column">
    ...
    <div class="mt-4 cards d-flex flex-wrap align-items-center justify-content-center gap-5">
        <?php if ($goods_result && $goods_result->num_rows > 0): ?>
        <?php while ($good = $goods_result->fetch_assoc()): ?>
            <div class="card" style="width: 18rem;">
                <img src="<?php echo htmlspecialchars($good['image']); ?>" class="card-img-top" alt="">
                <div class="card-body">
                    <h5 class="card-title">الاسم: <?php echo $good['name'];?></h5>
                    <p class="card-text">المكان: <?php echo $good['place'];?></p>
                    <p class="card-text">المصدر: <?php echo $good['exporter'];?></p>
                    <p class="card-text">الكمية: <?php echo $good['quantity'];?></p>
                    <p class="card-text">السعر: $<?php echo $good['price'] ?></p>
                    <a href="<?php echo htmlspecialchars("/company/navigationsgoods/Informationgood/informationgood.php?good_id=" . $good['id']);?>" class="btn btn-primary">عرض المعلومات</a>
                </div>
            </div>
        <?php endwhile;?>
        <?php else: ?>
